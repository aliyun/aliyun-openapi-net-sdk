    ...
    public class ListDevicePermitsResponseUnmarshaller
    {
        public static ListDevicePermitsResponse Unmarshall(UnmarshallerContext context)
        {
            ListDevicePermitsResponse listDevicePermitsResponse = new ListDevicePermitsResponse()
            {
                ...
                RequestId = context.StringValue("ListDevicePermits.RequestId"),
                Success = context.BooleanValue("ListDevicePermits.Success"),
                ErrorMessage = context.StringValue("ListDevicePermits.ErrorMessage")
            };
			...
			for (int i = 0; i < context.Length("ListDevicePermits.DevicePermissions.Length"); i++) {
                ...
                {
                    Id = context.LongValue($"ListDevicePermits.DevicePermissions[{i}].Id"),
                    AppKey = context.LongValue($"ListDevicePermits.DevicePermissions[{i}].AppKey"),
                    DeviceId = context.StringValue($"ListDevicePermits.DevicePermissions[{i}].DeviceId"),
                    GrantType = context.StringValue($"ListDevicePermits.DevicePermissions[{i}].GrantType"),
                    TopicFullName = context.StringValue($"ListDevicePermits.DevicePermissions[{i}].TopicFullName"),
                    TopicUserId = context.LongValue($"ListDevicePermits.DevicePermissions[{i}].TopicUserId")
                };
