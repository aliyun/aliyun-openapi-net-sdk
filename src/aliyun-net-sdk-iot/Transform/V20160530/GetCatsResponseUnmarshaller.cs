    ...
    public class GetCatsResponseUnmarshaller
    {
        public static GetCatsResponse Unmarshall(UnmarshallerContext context)
        {
            GetCatsResponse getCatsResponse = new GetCatsResponse()
            {
                ...
                RequestId = context.StringValue("GetCats.RequestId"),
                Success = context.BooleanValue("GetCats.Success"),
                ErrorMessage = context.StringValue("GetCats.ErrorMessage")
            };
            GetCatsResponse.CatInfo_ catInfo = new GetCatsResponse.CatInfo_()
            {
                Id = context.LongValue("GetCats.CatInfo.Id"),
                Name = context.StringValue("GetCats.CatInfo.Name"),
                HasChild = context.BooleanValue("GetCats.CatInfo.HasChild")
            };
			...
			for (int i = 0; i < context.Length("GetCats.CatInfo.Childs.Length"); i++) {
                ...
                {
                    Id = context.LongValue($"GetCats.CatInfo.Childs[{i}].Id"),
                    Name = context.StringValue($"GetCats.CatInfo.Childs[{i}].Name"),
                    HasChild = context.BooleanValue($"GetCats.CatInfo.Childs[{i}].HasChild")
                };
                childs.Add(childCat);
			}
