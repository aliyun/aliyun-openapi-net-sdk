    ...
    public class QueryPushDataResponseUnmarshaller
    {
        public static QueryPushDataResponse Unmarshall(UnmarshallerContext context)
        {
            QueryPushDataResponse queryPushDataResponse = new QueryPushDataResponse()
            {
                ...
                RequestId = context.StringValue("QueryPushData.RequestId")
            };
			...
			for (int i = 0; i < context.Length("QueryPushData.PushDataInfos.Length"); i++) {
                ...
                {
                    Date = context.StringValue($"QueryPushData.PushDataInfos[{i}].Date"),
                    Accepted = context.IntegerValue($"QueryPushData.PushDataInfos[{i}].Accepted"),
                    Sent = context.IntegerValue($"QueryPushData.PushDataInfos[{i}].Sent"),
                    Received = context.IntegerValue($"QueryPushData.PushDataInfos[{i}].Received"),
                    Opened = context.IntegerValue($"QueryPushData.PushDataInfos[{i}].Opened"),
                    Deleted = context.IntegerValue($"QueryPushData.PushDataInfos[{i}].Deleted")
                };
