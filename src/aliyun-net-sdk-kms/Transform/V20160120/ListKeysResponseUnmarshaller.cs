    ...
    public class ListKeysResponseUnmarshaller
    {
        public static ListKeysResponse Unmarshall(UnmarshallerContext context)
        {
            ListKeysResponse listKeysResponse = new ListKeysResponse()
            {
                ...
                TotalCount = context.IntegerValue("ListKeys.TotalCount"),
                PageNumber = context.IntegerValue("ListKeys.PageNumber"),
                PageSize = context.IntegerValue("ListKeys.PageSize"),
                RequestId = context.StringValue("ListKeys.RequestId")
            };
			...
			for (int i = 0; i < context.Length("ListKeys.Keys.Length"); i++) {
                ...
                {
                    KeyId = context.StringValue($"ListKeys.Keys[{i}].KeyId")
                };
                keys.Add(key);
			}
