    ...
    public class DescribeLiveStreamsControlHistoryResponseUnmarshaller
    {
        public static DescribeLiveStreamsControlHistoryResponse Unmarshall(UnmarshallerContext context)
        {
            DescribeLiveStreamsControlHistoryResponse describeLiveStreamsControlHistoryResponse = new DescribeLiveStreamsControlHistoryResponse()
            {
                ...
                RequestId = context.StringValue("DescribeLiveStreamsControlHistory.RequestId")
            };
			...
			for (int i = 0; i < context.Length("DescribeLiveStreamsControlHistory.ControlInfo.Length"); i++) {
                ...
                {
                    StreamName = context.StringValue($"DescribeLiveStreamsControlHistory.ControlInfo[{i}].StreamName"),
                    ClientIp = context.StringValue($"DescribeLiveStreamsControlHistory.ControlInfo[{i}].ClientIP"),
                    Action = context.StringValue($"DescribeLiveStreamsControlHistory.ControlInfo[{i}].Action"),
                    TimeStamp = context.StringValue($"DescribeLiveStreamsControlHistory.ControlInfo[{i}].TimeStamp")
                };
