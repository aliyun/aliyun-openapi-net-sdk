    ...
    public class DescribeLiveStreamPushDataResponseUnmarshaller
    {
        public static DescribeLiveStreamPushDataResponse Unmarshall(UnmarshallerContext context)
        {
            DescribeLiveStreamPushDataResponse describeLiveStreamPushDataResponse = new DescribeLiveStreamPushDataResponse()
            {
                ...
                RequestId = context.StringValue("DescribeLiveStreamPushData.RequestId")
            };
			...
			for (int i = 0; i < context.Length("DescribeLiveStreamPushData.PushStreamModelList.Length"); i++) {
                ...
                {
                    Time = context.StringValue($"DescribeLiveStreamPushData.PushStreamModelList[{i}].Time"),
                    FrameRate = context.FloatValue($"DescribeLiveStreamPushData.PushStreamModelList[{i}].FrameRate"),
                    BitRate = context.FloatValue($"DescribeLiveStreamPushData.PushStreamModelList[{i}].BitRate"),
                    FrameLossRate = context.FloatValue($"DescribeLiveStreamPushData.PushStreamModelList[{i}].FrameLossRate"),
                    ServerAddr = context.StringValue($"DescribeLiveStreamPushData.PushStreamModelList[{i}].ServerAddr"),
                    ClientAddr = context.StringValue($"DescribeLiveStreamPushData.PushStreamModelList[{i}].ClientAddr")
                };
