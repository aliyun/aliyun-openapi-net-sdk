...
namespace Aliyun.Acs.OpenSearch.Transform.V20171225
{
    ...
    {
        public static ListAppGroupErrorsResponse Unmarshall(UnmarshallerContext _ctx)
        {
			...
			for (int i = 0; i < _ctx.Length("ListAppGroupErrors.Result.Length"); i++) {
				...
				foreach (var _item in _ctx.ResponseDictionary){
					string prefix = "ListAppGroupErrors.Result["+ i +"].";
					if (_item.Key.IndexOf(prefix) == 0){
						tmp.Add(_item.Key.Substring(prefix.Length), _item.Value);
					}
				}
				if (tmp.Count > 0){
					listAppGroupErrorsResponse_result.Add(tmp);
				}
			}
			listAppGroupErrorsResponse.Result = listAppGroupErrorsResponse_result;
