...
namespace Aliyun.Acs.OpenSearch.Transform.V20171225
{
    public class ListDataSourceTablesResponseUnmarshaller
    {
        public static ListDataSourceTablesResponse Unmarshall(UnmarshallerContext _ctx)
        {
			...
			List<string> listDataSourceTablesResponse_result = new List<string>();
			for (int i = 0; i < _ctx.Length("ListDataSourceTables.Result.Length"); i++) {
				listDataSourceTablesResponse_result.Add(_ctx.StringValue("ListDataSourceTables.Result["+ i +"]"));
			}
			listDataSourceTablesResponse.Result = listDataSourceTablesResponse_result;
