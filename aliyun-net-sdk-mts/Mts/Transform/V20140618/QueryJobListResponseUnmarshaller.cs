    ...
    public class QueryJobListResponseUnmarshaller
    {
        public static QueryJobListResponse Unmarshall(UnmarshallerContext context)
        {
			...
			for (int i = 0; i < context.Length("QueryJobList.JobList.Length"); i++) {
				...
				QueryJobListResponse.Job.Output_.TransFeatures_ transFeatures = new QueryJobListResponse.Job.Output_.TransFeatures_();

				List<QueryJobListResponse.Job.Output_.TransFeatures_.Merge> mergeList = new List<QueryJobListResponse.Job.Output_.TransFeatures_.Merge>();
				for (int j = 0; j < context.Length("QueryJobList.JobList["+ i +"].Output.TransFeatures.MergeList.Length"); j++) {
					QueryJobListResponse.Job.Output_.TransFeatures_.Merge merge = new QueryJobListResponse.Job.Output_.TransFeatures_.Merge();
					merge.MergeURL = context.StringValue("QueryJobList.JobList["+ i +"].Output.TransFeatures.MergeList["+ j +"].MergeURL");
					merge.Start = context.StringValue("QueryJobList.JobList["+ i +"].Output.TransFeatures.MergeList["+ j +"].Start");
					merge.Duration = context.StringValue("QueryJobList.JobList["+ i +"].Output.TransFeatures.MergeList["+ j +"].Duration");
