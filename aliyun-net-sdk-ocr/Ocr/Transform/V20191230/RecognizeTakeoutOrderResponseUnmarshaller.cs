...
namespace Aliyun.Acs.ocr.Transform.V20191230
{
    ...
    {
        public static RecognizeTakeoutOrderResponse Unmarshall(UnmarshallerContext context)
        {
			RecognizeTakeoutOrderResponse recognizeTakeoutOrderResponse = new RecognizeTakeoutOrderResponse();

			recognizeTakeoutOrderResponse.HttpResponse = context.HttpResponse;
			recognizeTakeoutOrderResponse.RequestId = context.StringValue("RecognizeTakeoutOrder.RequestId");
			...
			for (int i = 0; i < context.Length("RecognizeTakeoutOrder.Data.Elements.Length"); i++) {
				...
				element.Score = context.FloatValue("RecognizeTakeoutOrder.Data.Elements["+ i +"].Score");
				element.Name = context.StringValue("RecognizeTakeoutOrder.Data.Elements["+ i +"].Name");
				element._Value = context.StringValue("RecognizeTakeoutOrder.Data.Elements["+ i +"].Value");

				List<string> element_boxes = new List<string>();
				for (int j = 0; j < context.Length("RecognizeTakeoutOrder.Data.Elements["+ i +"].Boxes.Length"); j++) {
					element_boxes.Add(context.StringValue("RecognizeTakeoutOrder.Data.Elements["+ i +"].Boxes["+ j +"]"));
				}
