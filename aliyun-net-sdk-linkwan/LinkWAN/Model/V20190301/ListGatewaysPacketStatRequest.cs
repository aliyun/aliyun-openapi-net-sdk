...
{
    public class ListGatewaysPacketStatRequest : RpcAcsRequest<ListGatewaysPacketStatResponse>
    {
        public ListGatewaysPacketStatRequest()
            : base("LinkWAN", "2019-03-01", "ListGatewaysPacketStat", "linkwan", "openAPI")
        {
		...
		private List<string> gwEuiLists = new List<string>(){ };
		...
		public List<string> GwEuiLists
		{
			...
			{
				gwEuiLists = value;
				for (int i = 0; i < gwEuiLists.Count; i++)
				{
					DictionaryUtil.Add(QueryParameters,"GwEuiList." + (i + 1) , gwEuiLists[i]);
				}
			}
		}
        ...
        public override ListGatewaysPacketStatResponse GetResponse(UnmarshallerContext unmarshallerContext)
        {
            return ListGatewaysPacketStatResponseUnmarshaller.Unmarshall(unmarshallerContext);
        }
    }
}
