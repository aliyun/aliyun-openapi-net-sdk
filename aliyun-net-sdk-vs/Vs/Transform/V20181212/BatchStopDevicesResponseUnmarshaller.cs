...
namespace Aliyun.Acs.vs.Transform.V20181212
{
    ...
    {
        public static BatchStopDevicesResponse Unmarshall(UnmarshallerContext _ctx)
        {
			BatchStopDevicesResponse batchStopDevicesResponse = new BatchStopDevicesResponse();

			batchStopDevicesResponse.HttpResponse = _ctx.HttpResponse;
			batchStopDevicesResponse.RequestId = _ctx.StringValue("BatchStopDevices.RequestId");
			...
			for (int i = 0; i < _ctx.Length("BatchStopDevices.Results.Length"); i++) {
				BatchStopDevicesResponse.BatchStopDevices_Result result = new BatchStopDevicesResponse.BatchStopDevices_Result();
				result.Id = _ctx.StringValue("BatchStopDevices.Results["+ i +"].Id");
				...
				for (int j = 0; j < _ctx.Length("BatchStopDevices.Results["+ i +"].Streams.Length"); j++) {
					...
					stream.Id = _ctx.StringValue("BatchStopDevices.Results["+ i +"].Streams["+ j +"].Id");
					stream.Name = _ctx.StringValue("BatchStopDevices.Results["+ i +"].Streams["+ j +"].Name");
					stream.Error = _ctx.StringValue("BatchStopDevices.Results["+ i +"].Streams["+ j +"].Error");
