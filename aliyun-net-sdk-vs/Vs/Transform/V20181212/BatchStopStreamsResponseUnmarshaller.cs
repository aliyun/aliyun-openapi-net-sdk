...
namespace Aliyun.Acs.vs.Transform.V20181212
{
    ...
    {
        public static BatchStopStreamsResponse Unmarshall(UnmarshallerContext context)
        {
			BatchStopStreamsResponse batchStopStreamsResponse = new BatchStopStreamsResponse();

			batchStopStreamsResponse.HttpResponse = context.HttpResponse;
			batchStopStreamsResponse.RequestId = context.StringValue("BatchStopStreams.RequestId");
			...
			for (int i = 0; i < context.Length("BatchStopStreams.Results.Length"); i++) {
				BatchStopStreamsResponse.BatchStopStreams_Result result = new BatchStopStreamsResponse.BatchStopStreams_Result();
				result.Id = context.StringValue("BatchStopStreams.Results["+ i +"].Id");
				result.Name = context.StringValue("BatchStopStreams.Results["+ i +"].Name");
				result.Error = context.StringValue("BatchStopStreams.Results["+ i +"].Error");
