...
namespace Aliyun.Acs.vs.Transform.V20181212
{
    ...
    {
        public static BatchResumeVsStreamResponse Unmarshall(UnmarshallerContext _ctx)
        {
			BatchResumeVsStreamResponse batchResumeVsStreamResponse = new BatchResumeVsStreamResponse();

			batchResumeVsStreamResponse.HttpResponse = _ctx.HttpResponse;
			batchResumeVsStreamResponse.RequestId = _ctx.StringValue("BatchResumeVsStream.RequestId");
			...
			for (int i = 0; i < _ctx.Length("BatchResumeVsStream.ResumeResult.Length"); i++) {
				...
				resumeResultInfo.Result = _ctx.StringValue("BatchResumeVsStream.ResumeResult["+ i +"].Result");
				resumeResultInfo.Detail = _ctx.StringValue("BatchResumeVsStream.ResumeResult["+ i +"].Detail");
				resumeResultInfo.Count = _ctx.IntegerValue("BatchResumeVsStream.ResumeResult["+ i +"].Count");

				List<string> resumeResultInfo_channels = new List<string>();
				for (int j = 0; j < _ctx.Length("BatchResumeVsStream.ResumeResult["+ i +"].Channels.Length"); j++) {
					resumeResultInfo_channels.Add(_ctx.StringValue("BatchResumeVsStream.ResumeResult["+ i +"].Channels["+ j +"]"));
				}
