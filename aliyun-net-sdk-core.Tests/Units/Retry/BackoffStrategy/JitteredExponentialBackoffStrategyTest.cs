...
{
    public class JitteredExponentialBackoffStrategyTest
    {
        [Fact]
        public void TestJitteredExponential()
        {
            var retryPolicyContext = new RetryPolicyContext(null, "200", 2, "ecs", "2019-06-01",
                "DescribeInstances", RetryCondition.BlankStatus);

            var jitterExponential = new JitteredExponentialBackoffStrategy(100, 2000);

            var delay = jitterExponential.GetDelayTimeBeforeNextRetry(retryPolicyContext);
