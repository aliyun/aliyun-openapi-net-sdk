...
{
    public class NoRetryConditionTest
    {
        [Fact]
        public void TestNoRetryCondition()
        {
            var retryPolicyContext = new RetryPolicyContext(null, "200", 3, "ecs", "2019-06-01",
                "DescribeInstance", RetryCondition.BlankStatus);

            var noRetryCondition = new NoRetryCondition();
            var shouldRetry = noRetryCondition.ShouldRetry(retryPolicyContext);
