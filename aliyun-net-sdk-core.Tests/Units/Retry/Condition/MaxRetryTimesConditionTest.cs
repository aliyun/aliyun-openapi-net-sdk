...
{
    public class MaxRetryTimesConditionTest
    {
        private readonly int maxRetryTimes = 1;

        [Fact]
        public void TestMaxRetryTimes()
        {
            var retryPolicyContext = new RetryPolicyContext(null, "200", 0, "ecs", "2019-06-01",
                "DescribeInstances", RetryCondition.BlankStatus);

            var maxRetryTimesCondition = new MaxRetryTimesCondition(maxRetryTimes);

            var shouldRetry = maxRetryTimesCondition.ShouldRetry(retryPolicyContext);
            Assert.Equal(RetryCondition.ShouldRetry, shouldRetry);

            shouldRetry = maxRetryTimesCondition.ShouldRetry(retryPolicyContext);
