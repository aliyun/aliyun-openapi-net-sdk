/*
 * Licensed to the Apache Software Foundation (ASF) under one
 * or more contributor license agreements.  See the NOTICE file
 * distributed with this work for additional information
 * regarding copyright ownership.  The ASF licenses this file
 * to you under the Apache License, Version 2.0 (the
 * "License"); you may not use this file except in compliance
 * with the License.  You may obtain a copy of the License at
 *
 *     http://www.apache.org/licenses/LICENSE-2.0
 *
 * Unless required by applicable law or agreed to in writing,
 * software distributed under the License is distributed on an
 * "AS IS" BASIS, WITHOUT WARRANTIES OR CONDITIONS OF ANY
 * KIND, either express or implied.  See the License for the
 * specific language governing permissions and limitations
 * under the License.
 */
using System.Collections.Generic;
using Newtonsoft.Json;

using Aliyun.Acs.Core;
using Aliyun.Acs.Core.Http;
using Aliyun.Acs.Core.Transform;
using Aliyun.Acs.Core.Utils;
using Aliyun.Acs.eds_user.Transform;
using Aliyun.Acs.eds_user.Transform.V20210308;

namespace Aliyun.Acs.eds_user.Model.V20210308
{
    public class UpdatePropertyRequest : RpcAcsRequest<UpdatePropertyResponse>
    {
        public UpdatePropertyRequest()
            : base("eds-user", "2021-03-08", "UpdateProperty", "eds-user", "openAPI")
        {
            if (this.GetType().GetProperty("ProductEndpointMap") != null && this.GetType().GetProperty("ProductEndpointType") != null)
            {
                this.GetType().GetProperty("ProductEndpointMap").SetValue(this, Aliyun.Acs.eds_user.Endpoint.endpointMap, null);
                this.GetType().GetProperty("ProductEndpointType").SetValue(this, Aliyun.Acs.eds_user.Endpoint.endpointRegionalType, null);
            }
			Method = MethodType.POST;
        }

		private string propertyKey;

		private long? propertyId;

		private List<long?> propertyValuess = new List<long?>(){ };

		[JsonProperty(PropertyName = "PropertyKey")]
		public string PropertyKey
		{
			get
			{
				return propertyKey;
			}
			set	
			{
				propertyKey = value;
				DictionaryUtil.Add(BodyParameters, "PropertyKey", value);
			}
		}

		[JsonProperty(PropertyName = "PropertyId")]
		public long? PropertyId
		{
			get
			{
				return propertyId;
			}
			set	
			{
				propertyId = value;
				DictionaryUtil.Add(BodyParameters, "PropertyId", value.ToString());
			}
		}

		[JsonProperty(PropertyName = "PropertyValues")]
		public List<long?> PropertyValuess
		{
			get
			{
				return propertyValuess;
			}

			set
			{
				propertyValuess = value;
				if(propertyValuess != null)
				{
					for (int depth1 = 0; depth1 < propertyValuess.Count; depth1++)
					{
						DictionaryUtil.Add(BodyParameters,"PropertyValues." + (depth1 + 1), propertyValuess[depth1]);
						DictionaryUtil.Add(BodyParameters,"PropertyValues." + (depth1 + 1), propertyValuess[depth1]);
					}
				}
			}
		}

		public class PropertyValues
		{

			private long? propertyValueId;

			private string propertyValue;

			[JsonProperty(PropertyName = "PropertyValueId")]
			public long? PropertyValueId
			{
				get
				{
					return propertyValueId;
				}
				set	
				{
					propertyValueId = value;
				}
			}

			[JsonProperty(PropertyName = "PropertyValue")]
			public string PropertyValue
			{
				get
				{
					return propertyValue;
				}
				set	
				{
					propertyValue = value;
				}
			}
		}

		public override bool CheckShowJsonItemName()
		{
			return false;
		}

        public override UpdatePropertyResponse GetResponse(UnmarshallerContext unmarshallerContext)
        {
            return UpdatePropertyResponseUnmarshaller.Unmarshall(unmarshallerContext);
        }
    }
}
