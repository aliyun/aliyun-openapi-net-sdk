...
namespace Aliyun.Acs.Iot.Transform.V20160530
{
    public class ListRuleResponseUnmarshaller
    {
        public static ListRuleResponse Unmarshall(UnmarshallerContext context)
        {
			...
			List<ListRuleResponse.RuleInfo> data = new List<ListRuleResponse.RuleInfo>();
			for (int i = 0; i < context.Length("ListRule.Data.Length"); i++) {
				...
				ruleInfo.Where = context.StringValue("ListRule.Data["+ i +"].Where");
				...
				ruleInfo.ProductKey = context.LongValue("ListRule.Data["+ i +"].ProductKey");
