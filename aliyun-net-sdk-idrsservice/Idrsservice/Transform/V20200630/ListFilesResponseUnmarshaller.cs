...
namespace Aliyun.Acs.idrsservice.Transform.V20200630
{
    public class ListFilesResponseUnmarshaller
    {
        public static ListFilesResponse Unmarshall(UnmarshallerContext _ctx)
        {
			...
			listFilesResponse.Code = _ctx.StringValue("ListFiles.Code");
			listFilesResponse.Message = _ctx.StringValue("ListFiles.Message");
			listFilesResponse.RequestId = _ctx.StringValue("ListFiles.RequestId");

			List<string> listFilesResponse_data = new List<string>();
			for (int i = 0; i < _ctx.Length("ListFiles.Data.Length"); i++) {
				listFilesResponse_data.Add(_ctx.StringValue("ListFiles.Data["+ i +"]"));
			}
			listFilesResponse.Data = listFilesResponse_data;
