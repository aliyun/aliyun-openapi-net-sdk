...
namespace Aliyun.Acs.Core
{
    public class DefaultAcsClient : IAcsClient
    {
        ...
        public async Task<T> GetAcsResponseAsync<T>(AcsRequest<T> request, CancellationToken cancellationToken) where T : AcsResponse
        {
            var httpResponse = await DoActionAsync(request, cancellationToken).ConfigureAwait(false);
            return ParseAcsResponse(request, httpResponse);
        }

        public async Task<T> GetAcsResponseAsync<T>(AcsRequest<T> request, bool autoRetry, int maxRetryNumber, CancellationToken cancellationToken) where T : AcsResponse
        {
            var httpResponse = await DoActionAsync(request, autoRetry, maxRetryNumber, cancellationToken).ConfigureAwait(false);
            return ParseAcsResponse(request, httpResponse);
        }

        public async Task<T> GetAcsResponseAsync<T>(AcsRequest<T> request, IClientProfile profile, CancellationToken cancellationToken) where T : AcsResponse
        {
            var httpResponse = await DoActionAsync(request, profile, cancellationToken).ConfigureAwait(false);
            return ParseAcsResponse(request, httpResponse);
        }

        public async Task<T> GetAcsResponseAsync<T>(AcsRequest<T> request, string regionId, Credential credential, CancellationToken cancellationToken)
                where T : AcsResponse
        {
        ...
        public async Task<CommonResponse> GetCommonResponseAsync(CommonRequest request, CancellationToken cancellationToken)
        {
        ...
        public Task<HttpResponse> DoActionAsync<T>(AcsRequest<T> request, CancellationToken cancellationToken) where T : AcsResponse
        {
            return DoActionAsync(request, AutoRetry, MaxRetryNumber, clientProfile, cancellationToken);
        }

        public Task<HttpResponse> DoActionAsync<T>(AcsRequest<T> request, bool autoRetry, int maxRetryNumber, CancellationToken cancellationToken)
        where T : AcsResponse
        {
            return DoActionAsync(request, autoRetry, maxRetryNumber, clientProfile, cancellationToken);
        }

        public Task<HttpResponse> DoActionAsync<T>(AcsRequest<T> request, IClientProfile profile, CancellationToken cancellationToken) where T : AcsResponse
        {
        ...
        public Task<HttpResponse> DoActionAsync<T>(AcsRequest<T> request, bool autoRetry, int maxRetryNumber, string regionId,
            Credential credential, Signer signer, FormatType? format, List<Endpoint> endpoints, CancellationToken cancellationToken) where T : AcsResponse
        {
